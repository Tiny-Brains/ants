//! What a model sees — the competitor guide's *What your model sees* is this view, in prose.
//!
//! Owners are relative to the observer: **you are always `0`**, an opponent is `1` upward. So the
//! same position, asked of either seat, comes back as the same bytes — which is what a self-play
//! trainer depends on.
//!
//! ```json
//! { "size":  [64, 96],
...
///
/// **This is the observer-relative rule, and for a long time this engine did not obey it.** It
/// emitted raw seat numbers, so seat 1's own hill arrived labelled `1` and every adapter that read
/// "owner 0 is mine" — including the reference one — had its own-hills and enemy-hills planes
/// swapped on one side of every match. Nothing caught it because the engine's own tests only ever
/// asked seat 0; `a_view_is_observer_relative` asks both.
///
