...
fn contested_food_is_unreachable_at_the_standard_radii() {
    ...
    for a in [(5, 6), (5, 4), (4, 5), (6, 5), (5, 5)] {
        for b in [(5, 6), (5, 4), (4, 5), (6, 5), (5, 5)] {
            ...
            assert!(
                m.g.dist2(at(&m, a.0, a.1), at(&m, b.0, b.1)) <= crate::grid::ATTACK_RADIUS2,
...
fn the_view_carries_the_mask_it_filtered_through() {
    // `vis` is sent rather than derived (`observe.rs`): it is the same bitmap `foes`, `food`
    ...
    // Lose every ant and the mask goes with the vision.
...
fn a_view_is_observer_relative() {
    // Relabel every seat in the world, ask the same player under its new label, and the bytes must
    // be identical. The engine used to emit raw seat numbers, so seat 1
    ...
    // And the labels are the ones the protocol promises: yourself 0, an opponent 1 upward.
