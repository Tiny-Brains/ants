...
fn a_map_survives_the_round_trip_to_a_file_and_back() {
    ...
    for p in crate::maps::PRESETS {
        ...
        let a = crate::maps::MapFile::from_match(&m, "round-trip", p.name);
        let b = crate::maps::MapFile::from_json(&a.to_json()).unwrap();
...
fn a_map_that_is_not_symmetric_is_refused_rather_than_played() {
    ...
    let base = crate::maps::pool("cell")[0].clone();
    let code = |mf: &crate::maps::MapFile| -> String {
    ...
    // A board that starts stocked and never restocks is allowed: `food::spawn` fills up to the
    ...
    let g = drowned.geom();
    let mut w = crate::grid::Bits::zeros(g.cells());
