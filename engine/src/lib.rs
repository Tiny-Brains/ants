...
//!
//! Five functions in one component, dispatched on the function name. `plugin.toml` declares each
//! one's input fields:
//!
//! ```text
...
//! tb.ants.replay-decode(payload, turn | from, to)  → frame | frames
//! ```
//!
//! Three things about that shape are load-bearing, each found by running it against a real Orion:
//!
