...
# (`cargo install wasm-tools`) and Python 3.11 or newer. The output is committed, so the package
# loads on a machine with no toolchain at all: run this after changing src/ or maps/, and commit
# what it produces with the change that caused it.
#
