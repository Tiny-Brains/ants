...
### The boards a season plays

| # | Question | Decision | What it overturns, and what it cost |
|---|---|---|---|
| N27 | Are a season's boards grown or drawn? | *Superseded in part by **N28** (soma), 19 September 2026: there are no presets, the component carries no boards, and a season's boards -- season 1's thirty-two among them -- are uploaded to it rather than committed here; what stands is how a board is drawn.* **Drawn. A board is a design** (`mapgen/recipes/<name>.toml`): its size, its seat shift, a decorative point group about every seat's centre, every shape drawn on it and every pattern's seed, rendered by `mapgen/src/design.rs`. `mapgen explore` proposes hundreds a slot from families of motifs placed where the lattice puts them (round each hill, on the borders' middles, where territories meet); a person picks by eye; `mapgen adopt` writes the pick down. **One board a preset**, named `<size>-<terrain>-<N>p-<H>h` — tiny, small, medium, large and xlarge by the longer side (32, 48, 64, 96, under 128), the shorter side never under the size below it allows. **A seat's hills are spread**: each in its own part of the territory with a base of its own, at least ten squares' walk apart and two fifths of the walk to the nearest enemy hill. Season 1 is thirty-two: eight a terrain, every terrain across every size, two to eight seats, one to four hills a seat | Decision **17 September 2026 (evening)**'s sixteen presets of four boards each, grown from area knobs as the runs of an L16 orthogonal array. The grown boards looked rolled — noise under a translation alone — and a seat's hills stood in one room, which makes a second hill a second door to the same base. What a person picked is kept as data rather than a seed, because a sampler improved later would otherwise redraw every board somebody chose. **Cost:** the catalogue is no longer balanced factor by factor (seat count and size are tied: a tiny board seats at most four, since no enemy hill may be in view at turn zero), the decorative symmetry falls to a half-turn or a mirror wherever the hill count forbids more, each preset is a pool of one so the seed no longer chooses among boards, and the area generator stays in `mapgen` for `sweep`, the fairness proof, which the designed boards do not use |
